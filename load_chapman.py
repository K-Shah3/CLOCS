...
chapman_denoised_file_path = os.path.join(chapman_file_path,'ECGDataDenoised')
print(chapman_file_path)
files = os.listdir(os.path.join(chapman_file_path,'ECGDataDenoised'))
# database = pd.read_csv(os.path.join(chapman_file_path,'Diagnostics.xlsx'))
database = pd.read_excel(os.path.join(chapman_file_path,'Diagnostics.xlsx'), 'Sheet1', dtype=str, index_col=None).applymap(lambda x: x.strip() if isinstance(x, str) else x)
...
# map rhythms to 4 rhythm classification
        ...
        for phase in phases:
            ...
            for term in terms:
                ...
                for patient in tqdm(current_patients):
                    ...
                    data = pd.read_csv(os.path.join(chapman_file_path,'ECGDataDenoised',filename)) #SxL
...
savepath = os.path.join(chapman_file_path,trial,'leads_%s' % str(desired_leads))
